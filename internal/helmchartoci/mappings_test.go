...
var _ = Describe("Image mappings", func() {
	...
	It("rejects values files outside the chart directory", func() {
		root := GinkgoT().TempDir()
		chartDir := filepath.Join(root, "chart")
		Expect(os.MkdirAll(chartDir, 0o755)).To(Succeed())
		Expect(os.WriteFile(filepath.Join(root, "outside.yaml"), []byte("image: localhost/app\n"), 0o644)).To(Succeed())

		mappings := []helmchartoci.ImageMapping{
			{Source: "localhost/app", Target: "quay.io/org/app"},
		}
		err := helmchartoci.ApplyImageMappings(chartDir, mappings, []string{"../outside.yaml"})
		Expect(err).To(MatchError(ContainSubstring("escapes chart directory")))
	})

	It("rejects absolute values file paths", func() {
		chartDir := GinkgoT().TempDir()
		mappings := []helmchartoci.ImageMapping{
			{Source: "localhost/app", Target: "quay.io/org/app"},
		}
		err := helmchartoci.ApplyImageMappings(chartDir, mappings, []string{"/tmp/values.yaml"})
...
`
	...
	It("stores mapped tags as YAML strings when updating an existing tag key", func() {
		chartDir := GinkgoT().TempDir()
		valuesYAML := `image:
  repository: registry.example:5000/team/app
  tag: 1.2
`
	...
	It("stores mapped tags as YAML strings when appending a new tag key", func() {
		chartDir := GinkgoT().TempDir()
		valuesYAML := `image:
...
`
		...
		mappings := []helmchartoci.ImageMapping{
			{Source: "quay.io/org/app:latest", Target: "quay.io/org/target:2.0"},
		}
		Expect(helmchartoci.ApplyImageMappings(chartDir, mappings, []string{"values.yaml"})).To(Succeed())
