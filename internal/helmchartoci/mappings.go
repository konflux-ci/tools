...
func ApplyImageMappings(chartDir string, mappings []ImageMapping, valuesFiles []string) error {
	...
	for _, valuesFile := range valuesFiles {
		path, err := valuesFilePath(chartDir, valuesFile)
		if err != nil {
			return err
		}
...
func valuesFilePath(chartDir, valuesFile string) (string, error) {
	if filepath.IsAbs(valuesFile) {
		return "", fmt.Errorf("values file %q must be relative to the chart directory", valuesFile)
	}

	absChartDir, err := filepath.Abs(chartDir)
	if err != nil {
		return "", fmt.Errorf("resolve chart directory: %w", err)
	}

	absPath, err := filepath.Abs(filepath.Join(absChartDir, valuesFile))
	if err != nil {
		return "", fmt.Errorf("resolve values file %q: %w", valuesFile, err)
	}

	rel, err := filepath.Rel(absChartDir, absPath)
	if err != nil {
		return "", fmt.Errorf("resolve values file %q: %w", valuesFile, err)
	}
	if rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
		return "", fmt.Errorf("values file %q escapes chart directory", valuesFile)
	}
